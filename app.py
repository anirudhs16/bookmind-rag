...
[data-testid="stFileUploaderDropzone"] button {
    background: #ffffff !important; color: #1a1a1a !important;
    border: 1px solid #d0d0cb !important; border-radius: 6px !important;
}
[data-testid="stFileUploaderDropzone"] small { color: #888888 !important; }
[data-testid="stSidebar"] button {
    ...
    font-size: 0.83rem !important; text-align: left !important;
    box-shadow: none !important; font-weight: 400 !important;
}
[data-testid="stSidebar"] button:hover { background: #ebebea !important; }
...
[data-testid="stTextInput"] input {
    background: #fafaf8 !important; border: 1px solid #e0e0da !important;
    border-radius: 10px !important; color: #1a1a1a !important;
    font-size: 0.9rem !important; padding: 10px 14px !important;
}
[data-testid="stTextInput"] input:focus {
    border-color: #aaa !important;
    box-shadow: 0 0 0 3px rgba(0,0,0,0.05) !important; outline: none !important;
}
...
.slabel {
    ...
    margin: 1rem 0 0.4rem; display: block;
}
.pill {
...
.p-gray { background: #f4f4f1; color: #555; border: 1px solid #e0e0da; }
.msg-user { display: flex; justify-content: flex-end; margin: 12px 0; }
.bubble-user {
    background: #1a1a1a; color: #f5f5f5; padding: 10px 15px;
    border-radius: 16px 4px 16px 16px; font-size: 0.88rem;
    line-height: 1.6; max-width: 76%; word-wrap: break-word;
}
...
.bot-avatar {
    ...
    justify-content: center; font-size: 13px; margin-top: 3px;
}
.bubble-bot {
    background: #fafaf8; border: 1px solid #e8e8e3; color: #1a1a1a;
    padding: 12px 16px; border-radius: 4px 16px 16px 16px;
    font-size: 0.88rem; line-height: 1.7; max-width: 85%; word-wrap: break-word;
}
.bubble-bot strong { font-weight: 600; color: #111; }
.src-row {
    display: flex; flex-wrap: wrap; gap: 5px;
    margin-top: 10px; padding-top: 10px; border-top: 1px solid #eeeee9;
}
.src-chip {
    display: inline-flex; align-items: center; gap: 3px;
    background: #f4f4f1; border: 1px solid #e5e5e0;
    border-radius: 20px; padding: 2px 9px;
    font-size: 0.69rem; color: #555; font-weight: 500;
}
.sug-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; max-width: 500px; margin: 0 auto; }
.sug-card { background: #fafaf8; border: 1px solid #e8e8e3; border-radius: 10px; padding: 14px; }
.sug-icon { font-size: 1.1rem; margin-bottom: 6px; }
.sug-text { font-size: 0.78rem; color: #111; font-weight: 500; line-height: 1.4; }
.sug-sub  { font-size: 0.68rem; color: #aaa; margin-top: 3px; }
.book-row {
    display: flex; align-items: center; justify-content: space-between;
    padding: 6px 8px; border: 1px solid #e8e8e3; border-radius: 8px;
    background: #fafaf8; margin-bottom: 5px;
}
.book-info { font-size: 0.78rem; color: #1a1a1a; font-weight: 500; }
.book-count { font-size: 0.68rem; color: #999; margin-top: 1px; }
</style>
...
for k, v in [
    ...
    ("rag_pipeline", None),
    ("db_sources", None),      # list of {source, count} loaded from Qdrant
    ("db_checked", False),     # have we checked Qdrant this session?
]:
...
def get_db_client():
    """Return the shared Qdrant client from the pipeline (init if needed)."""
    return get_pipeline().db


def refresh_db_sources():
    """Query Qdrant for what's indexed and cache in session state."""
    from rag.db import get_indexed_sources
    try:
        st.session_state.db_sources = get_indexed_sources(get_db_client())
    except Exception:
        st.session_state.db_sources = []
...
# ─────────────────────────────────────────────────────────────────────────────
# Auto-check Qdrant once per session (runs silently on first load)
# ─────────────────────────────────────────────────────────────────────────────
...
# Derive ready state from live Qdrant data (not session upload state)
rag_ready = bool(st.session_state.db_sources)
...
with st.sidebar:
    ...
    st.markdown(
        """<div style="display:flex;align-items:center;gap:10px;padding:0 0 14px 2px">
          <div style="background:#1a1a1a;width:30px;height:30px;border-radius:7px;flex-shrink:0;
    ...
    if not sessions_sorted:
        st.markdown('<p style="font-size:0.78rem;color:#aaa;margin:2px 0 8px">No conversations yet</p>',
                    unsafe_allow_html=True)
    ...
    # ── Books already in Qdrant ────────────────────────────────────────────
    st.markdown('<span class="slabel">Indexed Books</span>', unsafe_allow_html=True)

    sources = st.session_state.db_sources or []
        ...
        for src in sources:
            ...
            with col_book:
                st.markdown(
                    ...
                    f'<div><div class="book-info">📗 {fname[:26]}</div>'
                    f'<div class="book-count">{count:,} chunks</div></div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
            with col_del:
                if st.button("✕", key=f"delsrc_{fname}", help=f"Remove {fname} from index"):
                    with st.spinner(f"Removing {fname[:20]}…"):
                        from rag.db import delete_source
                        delete_source(get_db_client(), fname)
                    refresh_db_sources()
                    st.rerun()
    else:
        st.markdown('<p style="font-size:0.78rem;color:#aaa;margin:2px 0">No books indexed yet</p>',
                    unsafe_allow_html=True)

    # ── Upload new PDFs ────────────────────────────────────────────────────
    st.markdown('<span class="slabel">Upload New Book</span>', unsafe_allow_html=True)

    # Show existing source names so user knows what's already there
    ...
    # Only show new files that aren't already indexed
    new_files = [f for f in (uploaded or []) if f.name not in existing_names]
    already_indexed = [f for f in (uploaded or []) if f.name in existing_names]

    if already_indexed:
        for f in already_indexed:
            st.markdown(
                f'<div style="font-size:0.75rem;color:#15803d;padding:2px 0">'
                f'✓ {f.name[:28]} already indexed</div>',
                unsafe_allow_html=True,
            )
    ...
    # ── Manage / Clear ─────────────────────────────────────────────────────
    if sources:
        st.markdown('<span class="slabel">Manage Index</span>', unsafe_allow_html=True)
        from rag.db import total_points
        n_total = total_points(get_db_client())
        st.markdown(
            f'<div style="font-size:0.75rem;color:#888;margin-bottom:6px">'
            f'Total vectors in Qdrant: <strong>{n_total:,}</strong></div>',
            unsafe_allow_html=True,
        )
        if st.button("🗑  Clear entire index", use_container_width=True, key="clear_all_btn"):
            st.session_state["confirm_clear"] = True

        if st.session_state.get("confirm_clear"):
            st.warning("This will delete ALL indexed books permanently from Qdrant Cloud.")
            col_yes, col_no = st.columns(2)
            with col_yes:
                if st.button("Yes, delete all", key="confirm_yes", use_container_width=True):
                    with st.spinner("Clearing index…"):
                        from rag.db import delete_all
                        delete_all(get_db_client())
    ...
    st.markdown(
        ...
        f'<span class="pill {"p-ok" if gok else "p-err"}">{"✓" if gok else "✗"} Groq</span></div>'
        f'<div style="margin-bottom:5px">'
        f'<span class="pill {"p-ok" if qok else "p-err"}">{"✓" if qok else "✗"} Qdrant</span></div>'
        f'<div><span class="pill {"p-ok" if rag_ready else "p-gray"}">'
        f'{"✓ Ready" if rag_ready else "○ No books"}</span></div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        """<div style="text-align:center;padding:40px 20px 28px">
          ...
          <div style="font-size:0.84rem;color:#777;line-height:1.6">
            ...
            using hybrid search and cite every source.</div>
        </div>
        <div class="sug-grid">
          <div class="sug-card"><div class="sug-icon">💰</div>
            <div class="sug-text">What's the difference between assets and liabilities?</div>
            <div class="sug-sub">Rich Dad Poor Dad</div></div>
          <div class="sug-card"><div class="sug-icon">🛡️</div>
            <div class="sug-text">Explain the margin of safety concept</div>
            <div class="sug-sub">The Intelligent Investor</div></div>
          <div class="sug-card"><div class="sug-icon">⚖️</div>
            <div class="sug-text">Compare both authors' views on stock investing</div>
            <div class="sug-sub">Both books</div></div>
          <div class="sug-card"><div class="sug-icon">🧠</div>
...
for msg in session["messages"]:
    if msg["role"] == "user":
        ...
        st.markdown(
            f'<div class="msg-bot"><div class="bot-avatar">🤖</div>'
            f'<div class="bubble-bot">{safe_fmt(msg["content"])}{sources_html}</div></div>',
            unsafe_allow_html=True,
        )
        if msg.get("chart_data"):
            try:
                ...
                if vals and labs:
                    ...
                    if ctype == "bar":
                        bars = ax.bar(labs, vals, color=COLORS[:len(vals)], edgecolor="none", width=0.5)
                        for bar, val in zip(bars, vals):
                            ax.text(bar.get_x()+bar.get_width()/2,
                                    bar.get_height()+max(vals)*0.015,
                                    str(val), ha="center", va="bottom", color="#555", fontsize=8.5)
                    elif ctype == "line":
                        ax.plot(labs, vals, color="#1a1a1a", linewidth=2, marker="o",
                                markersize=5, markerfacecolor="#fff",
                        ...
                        ax.barh(labs, vals, color=COLORS[:len(vals)], edgecolor="none", height=0.5)
                    ax.set_title(cd.get("title",""), color="#1a1a1a", fontsize=10,
                                 pad=8, fontweight="600", loc="left")
                    ax.tick_params(colors="#888", labelsize=8)
                    for sp in ax.spines.values(): sp.set_edgecolor("#e8e8e3")
                    ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False)
                    ax.yaxis.grid(True, color="#f0f0ee", linewidth=0.8); ax.set_axisbelow(True)
...
# Chat input
# ─────────────────────────────────────────────────────────────────────────────
placeholder = "Ask about your books…" if rag_ready else "Index a PDF from the sidebar first…"
